...
#include <stdio.h>
#include <pthread.h>        // ptrhead程式庫的標頭檔
#include <unistd.h>         // sleep(1)的標頭檔
#include <semaphore.h>      // semaphore 的標頭檔
...
int main(void) {
    ...
    sem_init(&semaphore, 0, 0);     // 初始化旗標，僅用於本行程，初始值為 0
    /*
        旗標在使用前要先以 sem_init 初始化，
        第二個參數是指定是否要讓其他的行程（process）共用旗標，
        這裡我們是單一行程、多執行緒的程式，所以第二個參數設定為 0 即可；
        第三個參數則是設定旗標的初始值。
    */
    ...
    sem_post(&semaphore);       // 放入第1個新的工作（也就讓將計數器遞增 1）
    printf("-----1-----\n");

    sem_post(&semaphore);       // 放入第2個新的工作（也就讓將計數器遞增 1）
    printf("-----2-----\n");
    sleep(4);                   // 先休息，讓子執行緒做點事情
    ...
    sem_post(&semaphore);       // 放入第3個新的工作（也就讓將計數器遞增 1）
    printf("-----5-----\n");
